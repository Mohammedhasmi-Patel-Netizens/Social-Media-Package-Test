@extends('layouts.app')
@section('title', 'Post to Twitter (X)')
@section('content')

    <div class="glass-form-card p-8 rounded-3xl max-w-lg w-full text-slate-900 shadow-xl mt-8">
        <h1 class="text-2xl font-bold mb-6 text-slate-900 drop-shadow-sm text-center">Post to Twitter (X)</h1>
        ...
        <form action="{{ route('twitter.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                <label for="content" class="block text-sm font-medium text-slate-600">Tweet Content</label>
